...
def kmeans_ci(X=None, stand=None, weighting=None, prop=None, nclus=None, nsim=None):
    ...
    if prop != None:
        ...
        print(
            "Pre-filtering using EOF retaining the first "
            + str(a)
            + " components done ..."
        )
    ...
    for NC in range(0, 1, 1):
        ...
        for i in range(nsim):
            ...
            k[:, i] = KMeans(
                n_clusters=nclus, init="k-means++", n_init=1, max_iter=1000
            ).fit_predict(X)
            ...
            for j in range(nclus):
                ...
                if lj > 1:
                    ...
                    mean_cluster[j, :] = np.mean(
                        PC[np.nonzero(k[:, i] == j)[0], :], axis=0
                    )
                else:
        ...
        for i in range(nclus):
            ...
            for j in range(nsim):
                ...
                sample2 = MC[:, a].reshape(c, (nsim - 1) * nclus, order="F").copy()
        ...
        part = np.nonzero(np.mean(ACCmax, axis=0) == np.max(np.mean(ACCmax, axis=0)))[0]
